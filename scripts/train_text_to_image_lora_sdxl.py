...
def main(args):
    ...
    if args.dataset_name is not None:
        ...
        dataset = load_dataset(
            "json",
            data_files="/home/rhohen/Workspace/diffusion-env/data_precomputed_npy_pq/metadata.jsonl",
            cache_dir=args.cache_dir
        )
    ...
    train_resize = transforms.Lambda(lambd=lambda x: x)
    train_crop = transforms.CenterCrop(args.resolution) if args.center_crop else transforms.RandomCrop(args.resolution)
    # train_flip = transforms.RandomHorizontalFlip(p=1.0)
    train_flip = transforms.Lambda(lambd=lambda x: x)
    train_transforms = transforms.Compose(
        [
            # transforms.ToTensor(),
            transforms.Lambda(lambd= lambda x: np.load(f"{args.train_data_dir}/{x}")),
            transforms.Lambda(lambd= lambda x: torch.tensor(x, dtype=torch.float16)),
            transforms.Lambda(lambd= lambda x: x.permute(2, 0, 1)),
    ...
    def preprocess_train(examples):
        ...
        for image in images:
            image = train_transforms(image)
            # print(f"DEBUG: {image.shape}")
            y1, x1, h, w = train_crop.get_params(image, (512, 1024))
            image = crop(image, y1, x1, h, w)
